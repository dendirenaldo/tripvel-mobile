...
import 'package:tripvel/page/form_promo.dart';
...
class _PromoScreenState extends State<PromoScreen> with AutomaticKeepAliveClientMixin<PromoScreen> {
  @override
  ...
  Future<void> getPromo() async {
    ...
    final response = await http.get(
      Uri.parse(
          '${dotenv.env['RESTFUL_API']}/promo?offset=${(page - 1) * limit}&limit=$limit&order=$order${cari.text != '' ? '&search=${cari.text}' : ''}'),
          ...
          tempPromo.add(
            DataRow(
              cells: [
                ...
                DataCell(Text(GeneralFunctionality.tanggalIndonesiaPendek(data['tanggalBerlaku']))),
                DataCell(Text(data['tanggalBerlakuHingga'] != null
                    ? GeneralFunctionality.tanggalIndonesiaPendek(data['tanggalBerlakuHingga'])
                    : GeneralFunctionality.tanggalIndonesiaPendek(data['tanggalBerlaku']))),
                DataCell(Text('Rp${GeneralFunctionality.rupiah(data['minimalHarga'])}')),
                DataCell(
                  IconButton(
                    ...
                    onPressed: () => Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => FormPromoPage(refresh: getPromo, id: data['id']),
                      ),
                    ),
                  ),
                ),
  ...
  void initState() {
    ...
    column = [
      ...
      {
        'name': 'Tanggal Berlaku Hingga',
        'data': 'tanggalBerlakuHingga',
      },
      {
        'name': 'Minimal Harga',
        'data': 'minimalHarga',
                ...
                children: [
                  SizedBox(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Untuk melihat promo dengan lengkap, anda dapat menggeser ke kanan layar',
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            IconButton(onPressed: getPromo, icon: const Icon(Icons.refresh_rounded))
                          ],
                        )
                      ],
                    ),
                  ...
                  SizedBox(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        IconButton(
                            onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => FormPromoPage(refresh: getPromo))),
