...
import 'package:tripvel/page/form_kategori.dart';
...
class _KategoriScreenState extends State<KategoriScreen> with AutomaticKeepAliveClientMixin<KategoriScreen> {
  @override
  ...
  late List<DataRow> listKategori;
  late int jumlahData;
  late int totalData;
  late int totalPage;
  late int page;
  ...
  Future<void> getKategori() async {
    ...
    final response = await http.get(
      Uri.parse(
          '${dotenv.env['RESTFUL_API']}/kategori?offset=${(page - 1) * limit}&limit=$limit&order=$order${cari.text != '' ? '&search=${cari.text}' : ''}'),
    ...
    if (response.statusCode == 201 || response.statusCode == 200) {
      ...
      if (responseBody['data'].isNotEmpty) {
        final List<DataRow> tempKategori = [];
                ...
                DataCell(
                  IconButton(
                    ...
                    onPressed: () => Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => FormKategoriPage(refresh: getKategori, id: data['id']),
                      ),
                    ),
                  ),
                ),
        ...
        if (mounted) {
          setState(() {
            listKategori.addAll(tempKategori);
                ...
                children: [
                  SizedBox(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Untuk melihat kategori dengan lengkap, anda dapat menggeser ke kanan layar',
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            IconButton(onPressed: getKategori, icon: const Icon(Icons.refresh_rounded))
                          ],
                        )
                      ],
                    ),
                  ...
                  SizedBox(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        IconButton(
                            onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => FormKategoriPage(refresh: getKategori))),
