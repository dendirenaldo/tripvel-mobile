...
import 'package:tripvel/page/form_user.dart';
...
class _UserScreenState extends State<UserScreen> with AutomaticKeepAliveClientMixin<UserScreen> {
  ...
  Future<void> getUser() async {
    ...
    final response = await http.get(
      Uri.parse(
          '${dotenv.env['RESTFUL_API']}/account?offset=${(page - 1) * limit}&limit=$limit&order=$order${cari.text != '' ? '&search=${cari.text}' : ''}'),
          ...
          tempUser.add(
            DataRow(
              cells: [
                ...
                if (profile['role'] == 'Admin') DataCell(Text(data['travel'] != null ? data['travel']['nama'] : '-')),
                DataCell(
                  ClipOval(
                    child: Image.network(
                      '${dotenv.env['RESTFUL_API']}/account/foto-profil/${data['gambar']}',
                      width: 40,
                      height: 40,
                      fit: BoxFit.cover,
                ...
                DataCell(
                  IconButton(
                    ...
                    onPressed: () => Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => FormUserPage(refresh: getUser, id: data['id']),
                      ),
                    ),
                  ),
                ),
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      sortColumnIndex = profile['role'] == 'Admin' ? 1 : 2;
                ...
                children: [
                  SizedBox(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Untuk melihat user dengan lengkap, anda dapat menggeser ke kanan layar',
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            IconButton(onPressed: getUser, icon: const Icon(Icons.refresh_rounded))
                          ],
                        )
                      ],
                    ),
                  ...
                  SizedBox(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        IconButton(
                            onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => FormUserPage(refresh: getUser))),
